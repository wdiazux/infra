...
- **OIDC config drift**: CronJob updates redirect URIs, auth method, and grant types via PUT on every sync
...
- **Idempotent**: Safe to run multiple times, no unnecessary restarts or credential rotation
...
Paperless uses Django allauth for OIDC. Key configuration:

- **PAPERLESS_URL**: Must use `https://` (not `http://`). Django derives CSRF trusted origins from this value. With `http://`, the HTTPS origin from Cilium Gateway is rejected with a 403.
- **Env var ordering**: `OIDC_CLIENT_ID` and `OIDC_CLIENT_SECRET` must be defined BEFORE `PAPERLESS_SOCIALACCOUNT_PROVIDERS` in the deployment env list. Kubernetes `$(VAR)` substitution only works for previously-defined vars.
- **Account linking**: Requires both `PAPERLESS_SOCIAL_AUTO_SIGNUP=true` and `EMAIL_AUTHENTICATION=true` in the provider config. Without auto signup, allauth shows a signup form where the existing username conflicts.
- **Admin email**: `PAPERLESS_ADMIN_MAIL` must match the Zitadel email. Without it, the admin user gets `root@localhost` which prevents email-based account linking.

### Immich

Immich uses a config file (`immich.json`) for OIDC, generated by an init container:

- **Auth Method**: Must use `OIDC_APP_AUTH_METHOD_TYPE_POST` in Zitadel. Immich sends credentials as `client_secret_post`, not `client_secret_basic`.
- **Storage Label**: `storageLabelClaim` is set to `email` (uses Zitadel email as username).
- **Account linking**: Immich does not support automatic account linking. If the `oauthId` field in the database is stale (e.g., after Zitadel reset), clear it manually:
  ```bash
  kubectl exec -n media statefulset/immich-postgres -- \
    psql -U postgres -d immich -c "UPDATE \"user\" SET \"oauthId\" = '' WHERE email = 'root@wdiaz.org';"
  ```
